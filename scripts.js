...
function buyItem(itemName, itemCost) {
    if (money >= itemCost) {
        ...
        const buyButton = document.querySelector(`button[onclick="buyItem('${itemName}', ${itemCost})"]`);
        buyButton.textContent = 'Equip';
        buyButton.onclick = () => equipItem(itemName);

        // Show preview image
        const previewImage = buyButton.nextElementSibling;
...
function equipItem(itemName) {
    alert(`You have equipped ${itemName}!`);
    const previewImage = document.querySelector(`img[alt="${itemName} Preview"]`);
    const accessoryPreview = document.getElementById('accessory-preview');
    accessoryPreview.innerHTML = '';
    accessoryPreview.appendChild(previewImage.cloneNode(true));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const songs = [
        ...
        'Songs/Lunar Drive.mp3',
        'Songs/The Past, The Present.mp3',
        'Songs/Valentine.mp3',
        'Songs/Water & Sand.mp3'
